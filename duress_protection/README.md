...
Duress checks and signaling initiates and takes place in the boomlet as the trusted hardware component in our protocol. User interface to boomlet is the Secure Terminal or ST that is designed to minimize side channel attack surface against the duress mechanism in a non-isolated environment.
...
2. The Search And Rescue entity or SAR: Is the trusted entity that once received the duress signal, can decrypt doxing data and start the search and rescue operation of the user in duress.
...
2. Dynamic doxing data: This a live feed of choice that helps indicating user's last known location. Like a location feed provided by user's mobile phone.

All doxing data are encrypted by `doxing_password`. Static doxing data is indicated at setup stage and sent to SAR by user as part of the registration process with the SAR. Dynamic doxing data is encrypted at source by the `doxing_password` and continuously sent to SAR via the originating device.

Most communications with the SAR is done through the user's phone.

Once the duress signal is sent, boomlet sends a payload containing `doxing_password` encrypted for SAR. This is sent to WT via the next packet. WT sends it to SAR to receive an acknowledgement of reception.

In other cases (no duress signal issued), boomlet encrypts a dummy data in the same manner and puts it in the duress signal bearing part of each message. SAR always tries to decrypt the received data with its keys. If it can decrypt it to the key relevant to any of its customers and subsequently able to unlock the doxing data, it means there is duress.
...
8. If the attacker is to observe the user for 'n' rounds of duress checks, the probability of guessing non-duress pattern must be less than an acceptable amount which we name `P[n](ao)`.
9. If the attacker is to guess at random, let's say it has a chance of `P(ag)` to pass the duress check successfully.
10. If the attacker is to torture the user, ideally, the attacker must know that the user has a big enough set to choose from and every choice will be consistent with user's  and system's behavior in non-duress condition, in a way that adhering to those may cause duress signal with a probability of `P(ac)`. This should be high enough to prevent the attacker from initiating an attack under our assumptions.

## Attacker characteristics

1. Attacker can capture the user and does not leave.
2. Attacker can observe the user in captive.
3. Attacker can read messages sent by the user.
...
2. A duress bearing payload that exist in every message omitted by boomlet, will be replaced by the doxing password encrypted by boomlet and SAR keys (DH / AES).
...
2. The attacker cannot observe user's hand when using the device or cannot confer meaningful information without seeing the hand and the monitor at the same time.
...
Duress check is initiated by the boomlet at the commitment to the withdrawal transaction step and at verifiably random instances that are verifiable by the WTs as well.
...
On the other hand we can use consequent hashing as the VRF. This can be done based on an agreed upon and shared piece of data. Like a hash of the descriptor or a hash of all `boomlet_identity_pubkey`s.

A third option will be the PRNG inside boomlet, which makes the duress check instances unpredictable by any entity outside boomlet. This will cause issues though if we do the duress checks too frequently as people may not be present 24/7 to answer the checks and their geographic distance stipulates different time zones and prolonged delays as a result. Of course this is a thing for rather all methods stated here.
...
2. Boomlet will send an encrypted ordered list of numbers from 1 to 195 inclusive, via a QR code showed on Iso to ST.
3. ST will scan the QR code on Iso's monitor and show the list of countries assigned to the list in an ascending alphabetical order.
4. User will choose 5 countries at will. Order is not important.
...
4. Boomlet will export this encrypted data to niso.
...
17. If:
    ...
    17.1.1. Boomlet will encrypt `doxing_password` with SAR keys.
    ...
    17.1.6. SARs will double hash the resulted data and search if any of their customers share the same result as their identifier.
    17.1.7. If there is a match, the SAR knows its user is in duress and goes into the operational procedure of retrieval by decrypting static and dynamic doxing data by the `doxing_password`. If not, SAR will ignore the message.

    ##### 17.2. the user has not input a duress

    17.2.1. Boomlet encrypts a random data with with SAR keys.
    17.2.2. Boomlet will put the encrypted result in the duress bearing part of the next message.
    17.2.3. Boomlet will send the message to WTs.
    ...
    17.2.6. SARs will double hash the resulted data and search if any of their customers share the same result as their identifier.
    17.2.7. There would be no match and SAR will ignore the message.
